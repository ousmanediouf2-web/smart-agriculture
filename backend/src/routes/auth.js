...
router.post('/register', async (req, res) => {
  try {
    const { name, email, password, phone, telegramChatId, role } = req.body;
    ...
    const user = await User.create({ name, email, password, phone, telegramChatId, role: role || 'farmer' });
...
router.put('/profile', protect, async (req, res) => {
  try {
    const { name, phone, telegramChatId, notificationPrefs } = req.body;
    const user = await User.findByIdAndUpdate(
      req.user._id,
      { name, phone, telegramChatId, notificationPrefs },
