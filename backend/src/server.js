...
const { initTelegram, sendDailyReports } = require('./services/telegramService');
...
cron.schedule('*/5 * * * *', async () => {
  try {
    ...
    console.log(`🔍 Cron capteurs: ${offlineSensors.length} capteur(s) hors ligne détecté(s)`);
    ...
    for (const sensor of offlineSensors) {
      ...
      if (!recentAlert) {
        ...
        // Notifier l'agriculteur par Telegram — utilise sensor.userId en priorité
        // (toujours rempli) car sensor.parcelId peut être null si le capteur
        // n'a pas encore été assigné à une parcelle
        const ownerId = sensor.parcelId?.userId || sensor.userId;
        if (ownerId) {
          const User = require('./models/User');
          const user = await User.findById(ownerId);
          if (user?.telegramChatId && user?.notificationPrefs?.critical) {
            console.log(`📤 Envoi alerte Telegram à ${user.name} (${user.telegramChatId}) pour capteur ${sensor.deviceId}`);
            await sendAlert('sensor_offline', {
              deviceId: sensor.deviceId,
              parcelName: sensor.parcelId?.name || 'Non assignée',
              lastSeen: new Date(sensor.lastSeen).toLocaleString('fr-FR')
            }, user.telegramChatId);
          } else {
            console.log(`⏭️  Alerte capteur ${sensor.deviceId} non envoyée — telegramChatId: ${user?.telegramChatId ? 'OK' : 'MANQUANT'}, critical: ${user?.notificationPrefs?.critical}`);
          }
        } else {
          console.log(`⏭️  Capteur ${sensor.deviceId} sans propriétaire identifiable (ni parcelId.userId ni userId)`);
        }
...
// Rapport journalier Telegram à 7h00 chaque matin
cron.schedule('0 7 * * *', async () => {
  console.log('📱 Envoi des rapports journaliers Telegram...');
...
cron.schedule('0 8 */5 * *', async () => {
  try {
    ...
    for (const parcel of parcels) {
      ...
      if (!user?.telegramChatId || !user?.notificationPrefs?.sms) continue;

      await sendTelegram(user.telegramChatId, 'satellite_update', {
...
// ── NOTE sur le sommeil Render Free ─────────────────────────────────────────
// Un setInterval() interne NE PEUT PAS empêcher la mise en veille de Render Free :
// si le service s'endort par manque de requêtes HTTP ENTRANTES, le processus
// Node.js (et donc ce setInterval) s'arrête aussi — il ne peut pas se réveiller
// lui-même. La seule solution gratuite fiable est un ping EXTERNE régulier
// (UptimeRobot, cron-job.org...) qui envoie une vraie requête HTTP depuis
// l'extérieur vers /health toutes les 5-10 minutes. Voir README.md pour la config.
...
const start = async () => {
  ...
  server.listen(PORT, () => {
    ...
    console.log(`📱 Telegram Bot: ${process.env.TELEGRAM_BOT_TOKEN ? '✅' : '⚠️  non configuré'}`);
