      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: user.telegramChatId ? '#2563eb' : S.text3.color }}>
              {user.telegramChatId ? `Telegram: ${user.telegramChatId}` : 'Telegram non configuré'}
            </p>
          </div>
        </div>
      </div>
      ...
      {tab === 'parcels' && (
        <div className="space-y-2">
          {parcels.length === 0 ? (
            ...
            const parcelSensorCount = sensors.filter(s => (s.parcelId?._id || s.parcelId) === p._id).length;
            return (
            <div key={p._id} style={S.row}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs" style={S.text3}>{p.cropId?.label || p.cropType} • {getParcelStatusLabel(p.status)} • {parcelSensorCount} capteur(s)</p>
