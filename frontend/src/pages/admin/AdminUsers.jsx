...
import { Users, Plus, Edit2, Trash2, Key, Phone, MessageCircle, Search, X, Save, Loader, Eye, UserX, UserCheck, CheckCircle, XCircle, Cpu } from 'lucide-react';
...
const UserModal = ({ user, onClose, onSave }) => {
  const [form, setForm] = useState(user || {
    name: '', email: '', password: '', phone: '', telegramChatId: '', role: 'farmer',
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    const res = user
      ? await adminFetch('PUT', `/users/${user._id}`, { name: form.name, email: form.email, phone: form.phone, telegramChatId: form.telegramChatId, role: form.role, isActive: form.isActive, isValidated: form.isValidated, notificationPrefs: form.notificationPrefs })
      ...
      <div className="w-full max-w-md rounded-2xl p-6" style={S.card}>
        ...
        <div className="space-y-3">
          {[
            ...
            { label: 'Téléphone (optionnel)', key: 'phone', type: 'text', placeholder: '+221771234567' },
            { label: 'Chat ID Telegram', key: 'telegramChatId', type: 'text', placeholder: 'ex: 123456789' },
          ...
          <div>
            <label className="text-xs font-semibold uppercase tracking-wide mb-2 block" style={S.text3}>Notifications Telegram</label>
...
export default function AdminUsers() {
  ...
  const handleValidate = async (user) => {
    ...
    if (res.success) {
      ...
      // Notifier par Telegram si validation
      if (!user.isValidated && user.telegramChatId) {
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {u.telegramChatId
                        ? <p className="text-xs flex items-center gap-1 mt-0.5" style={{ color: '#2563eb' }}><MessageCircle size={9} />Telegram: {u.telegramChatId}</p>
                        : <p className="text-xs flex items-center gap-1 mt-0.5" style={{ color: 'var(--a-text3)' }}><MessageCircle size={9} />Telegram non configuré</p>}
