...
const NDVI_COLOR = { poor: '#dc2626', moderate: '#d97706', good: '#16a34a', excellent: '#059669' };
const NDVI_LABEL = { poor: 'Végétation faible', moderate: 'Végétation modérée', good: 'Végétation saine', excellent: 'Végétation excellente' };
              ...
              {satLoading ? (
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-2">
                    <div className="rounded-xl p-3" style={{ background: 'var(--f-surface2)', border: '1px solid var(--f-border)' }}>
                      <p className="text-xs" style={{ color: 'var(--f-text3)' }}>NDVI</p>
                      <p className="text-xl font-bold mt-0.5" style={{ color: NDVI_COLOR[satellite.ndviStatus] }}>{satellite.ndvi}</p>
                      <p className="text-xs mt-0.5" style={{ color: NDVI_COLOR[satellite.ndviStatus] }}>{NDVI_LABEL[satellite.ndviStatus]}</p>
                      <div className="mt-2 h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--f-border)' }}>
                        <div className="h-full rounded-full transition-all"
                          style={{ width: `${parseFloat(satellite.ndvi) * 100}%`, background: NDVI_COLOR[satellite.ndviStatus] }} />
                      </div>
                    </div>
                    <div className="rounded-xl p-3" style={{ background: 'var(--f-surface2)', border: '1px solid var(--f-border)' }}>
                      <p className="text-xs" style={{ color: 'var(--f-text3)' }}>Nuages</p>
                      <p className="text-xl font-bold mt-0.5" style={{ color: 'var(--f-text)' }}>{satellite.cloudCoverage}%</p>
                      <p className="text-xs mt-0.5" style={{ color: satellite.cloudCoverage < 20 ? '#16a34a' : '#d97706' }}>
                        {satellite.cloudCoverage < 20 ? '✅ Dégagé' : '⚠️ Nuageux'}
                      </p>
                    </div>
                  </div>
                  <div className="rounded-xl p-3 text-xs space-y-1.5" style={{ background: 'var(--f-surface2)', border: '1px solid var(--f-border)' }}>
                    ...
                    <div className="flex justify-between">
                      <span style={{ color: 'var(--f-text3)' }}>Prochaine</span>
                      <span className="font-medium" style={{ color: 'var(--f-primary)' }}>{format(new Date(satellite.nextUpdateAt), 'dd/MM/yyyy', { locale: fr })}</span>
                    </div>
                    <div className="flex justify-between">
                  ...
                  <div className="px-3 py-2 rounded-xl text-xs flex items-start gap-2"
                    style={{ background: 'var(--f-primary-light)', color: 'var(--f-primary)' }}>
                    <Info size={12} className="mt-0.5 flex-shrink-0" />
                    Image mise à jour toutes les 5 jours. Notification WhatsApp à chaque nouvelle image.
                  </div>
                </div>
              ) : null}
            </div>
