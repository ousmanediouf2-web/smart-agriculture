...
export default function DashboardPage() {
  ...
  const fetchSeries = async () => {
    try {
      const res = await apiFetch('/measures?limit=60');
      if (res.success) {
  ...
  return (
    <div className="space-y-6 page-transition">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 page-enter">
        <StatCard icon={Droplets} label="Humidité Sol" value={stats?.avgSoilHumidity} unit="%" iconBg="linear-gradient(135deg, #0ea5e9, #38bdf8)" delay="card-pop-1" />
        <StatCard icon={Thermometer} label="Température" value={stats?.avgTemperature} unit="°C" iconBg="linear-gradient(135deg, #f97316, #fb923c)" delay="card-pop-2" />
        <StatCard icon={Wind} label="Humidité Air" value={stats?.avgAirHumidity} unit="%" iconBg="linear-gradient(135deg, #14b8a6, #2dd4bf)" delay="card-pop-3" />
        <StatCard icon={Activity} label="Arrosages" value={stats?.pumpActivations} unit="fois" iconBg="linear-gradient(135deg, #8b5cf6, #a78bfa)" delay="card-pop-4" />
      </div>
