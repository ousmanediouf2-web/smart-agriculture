...
export default function SettingsPage() {
  ...
  const handleTestTelegram = async () => {
    if (profile.telegramChatId !== user?.telegramChatId) {
      toast.error('Enregistrez d\'abord vos modifications avant de tester.');
      return;
    }
    setTestingTelegram(true);
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`${BACKEND_URL}/api/users/me/test-telegram`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await res.json();
      if (data.success) toast.success('Message envoyé ! Vérifie ton Telegram 📲');
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="card">
        ...
        <div className="space-y-4 page-enter">
          ...
          <div>
            ...
            <input className="input" value={profile.telegramChatId}
              onChange={e => setProfile(p => ({ ...p, telegramChatId: e.target.value }))}
              placeholder="ex: 123456789" />
            <div className="text-xs text-gray-500 mt-2 bg-gray-800/50 rounded-lg p-3 space-y-1">
              <p className="font-semibold text-gray-400">📲 Comment obtenir votre Chat ID :</p>
              <p>1. Ouvrez Telegram et recherchez le bot <span className="text-blue-400">@AgroSmartBot</span></p>
              <p>2. Cliquez sur "Démarrer" ou envoyez <code className="text-green-400">/start</code></p>
              <p>3. Le bot vous répond avec votre Chat ID — collez-le ci-dessus</p>
            </div>
            {user?.telegramChatId && (
              <button onClick={handleTestTelegram} disabled={testingTelegram} type="button"
                className="mt-2 flex items-center gap-2 text-xs px-3 py-2 rounded-lg bg-blue-900/20 text-blue-400 hover:bg-blue-900/30 transition-colors disabled:opacity-50">
                {testingTelegram ? <Loader size={12} className="animate-spin" /> : <MessageCircle size={12} />}
                {testingTelegram ? 'Envoi...' : 'Tester la connexion Telegram'}
              </button>
            )}
          </div>
          <div>
      ...
      <div className="card">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="text-sm font-semibold text-gray-300">Notifications Telegram</h2>
        </div>
        ...
        {!profile.telegramChatId && (
          <div className="mt-4 bg-yellow-900/20 border border-yellow-700/30 rounded-lg p-3">
            <p className="text-xs text-yellow-400">⚠️ Ajoutez votre Chat ID Telegram ci-dessus pour recevoir les notifications.</p>
          </div>
        )}
      </div>
